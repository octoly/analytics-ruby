      ...
      describe Worker do
        describe "#init" do
          it 'accepts string keys' do
            queue = Queue.new
            worker = Octoly::Segment::Analytics::Worker.new(queue, 'secret', 'batch_size' => 100)
        ...
        describe '#run' do
          ...
          it 'executes the error handler, before the request phase ends, if the request is invalid' do
            Octoly::Segment::Analytics::Request.any_instance.stub(:post).and_return(Octoly::Segment::Analytics::Response.new(400, "Some error"))
            ...
            queue = Queue.new
            queue << {}
            worker = Octoly::Segment::Analytics::Worker.new queue, 'secret', :on_error => on_error
